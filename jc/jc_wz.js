...
if (jc_timebody) {
    ...
    var fs = require("fs");
    jc_timebody = fs.readFileSync("jc_timebody.txt", "utf8");
...
if (wzbody) {
    ...
    var fs = require("fs");
    wzbody = fs.readFileSync("jcwzbody.txt", "utf8");
    if (wzbody !== `undefined`) {
        wzbodys = wzbody.split("\n");
    }
} else {
...
!(async () => {
    if (typeof $request !== "undefined") {
        ...
        index1 = indexLast * 1
        for (let k = index1; k < wzbodyArr.length; k++) {
            // $.message = ""
            wzbody1 = wzbodyArr[k];
            // console.log(`${wzbody1}`)
