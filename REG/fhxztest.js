...
function test(){
    for(i=1;i<=9;i++){
       var harvest= `'[{"type":"farmland_harvest","data":{"farmlandDefId":${i}}}]'`
       $.log(harvest)
    }
}
  ...
  return new Promise((resolve) => {
    ...
    $.post(url, async(err, resp, data) => {
      try {
        ...
        var lb = data
        var label, target = ""
        var num = lb[0]["data"].numOfReceived
        num = Number(num)
        num==8 ? um=10 : num
        for (let i = 0; i < num; i++) {
  ...
  return new Promise((resolve) => {
    for(let i=1;i<9;i++){
        ...
        let url = {
          ...
          body: `'[{"type":"farmland_harvest","data":{"farmlandDefId":${i}}}]'`,
        }
        $.post(url, async(err, resp, data) => {
          try {
