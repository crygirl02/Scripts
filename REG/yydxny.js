              ...
              oemharvests.forEach(async function(oemharvestsitem) {
                if (oemharvestsitem.index == OrderListNextItem.index) {
                  ...
                  if(oemharvestsitem.count<OrderListNextItem.count){
                    await makeKitchenAll(arr,OrderListNextItem.index)
                  }
                }
              })              
            })
  ...
  return new Promise((resolve) =>{
    ...
    $.post(nm, async(error, response, data) => {
      try {
        ...
        if (logs) $.log(data)
        if (result.code == 0) {
          console.log(`${landIndex+1}号地播种成功，成熟时间：${result.result.landMessage.countdown}秒`)
        } else {
          console.log(`${landIndex+1}号地${result.message}`)
        }
      } catch (e) {
  ...
  return new Promise((resolve) => {
    ...
    $.post(nm, async(error, response, data) => {
      try {
        const result = JSON.parse(data)
        if (result.code == 0) {
          $.log(kitchenId+1 +"号厨房制作成功，成熟时间：" + result.result.kitchenMessage.countdown)
        } else {
