...
let bianliang = $.isNode ? (process.env.bianliang ? process.env.bianliang : "") : ($.getdata('bianliang') ? $.getdata('bianliang') : "")
let message = ""
let bianliangArr = []
...
function GetHeaders() {
  if ($request.url.indexOf('') > -1) {
    const bianliang = JSON.stringify($request.headers)
    let Headers = JSON.parse(bianliang)
    Headers = bianliang.match().toString()
    $.setdata(Headers, 'bianliang')
    $.msg($.name, "数据获取成功")
  }
}
